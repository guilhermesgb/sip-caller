 ...
 *
 * An arbitrary call is in the [com.xibasdev.sipcaller.sip.calling.CallStage.INVITATION] stage while
 *   it hasn't been accepted by the callee (the local party in the case of incoming calls or the
 *   remote party in case of outgoing calls) or if either caller decides to cancel, callee decides
 *   to decline, callee misses the invitation or a remote callee accepts it elsewhere.
 *
 * An arbitrary call transitions to [com.xibasdev.sipcaller.sip.calling.CallStage.SESSION] stage
 *   when it is accepted by the callee (the local party in the case of incoming calls or the remote
 *   party in case of outgoing calls). If the call session is later terminated by either local or
 *   remote party or if some error happens while the session was still in progress, the call remains
 *   in the session stage.
 *
 ...
 *
 * [com.xibasdev.sipcaller.sip.calling.CallStage.INVITATION] stage can have these status:
 * - [com.xibasdev.sipcaller.sip.calling.CallStatus.RINGING]
 * - [com.xibasdev.sipcaller.sip.calling.CallStatus.CANCELED] (terminal)
 * - [com.xibasdev.sipcaller.sip.calling.CallStatus.MISSED] (terminal)
 * - [com.xibasdev.sipcaller.sip.calling.CallStatus.ACCEPTED_ELSEWHERE] (terminal)
 * - [com.xibasdev.sipcaller.sip.calling.CallStatus.DECLINED] (terminal)
 *
 * [com.xibasdev.sipcaller.sip.calling.CallStage.SESSION] stage can have these status:
 * - [com.xibasdev.sipcaller.sip.calling.CallStatus.ACCEPTED]
 * - [com.xibasdev.sipcaller.sip.calling.CallStatus.FINISHED_BY_LOCAL_PARTY] (terminal)
 * - [com.xibasdev.sipcaller.sip.calling.CallStatus.FINISHED_BY_REMOTE_PARTY] (terminal)
 * - [com.xibasdev.sipcaller.sip.calling.CallStatus.FINISHED_DUE_TO_ERROR] (terminal)
 *
 ...
 *   [com.xibasdev.sipcaller.sip.calling.streams.MediaStream.direction], which can be one of the
 *   following:
 *
 *   - [com.xibasdev.sipcaller.sip.calling.streams.StreamDirection.DISABLED]: indicates that there's
 *       currently no data being exchanged in the corresponding media stream;
 *   - [com.xibasdev.sipcaller.sip.calling.streams.StreamDirection.ENABLED_RECEIVE_ONLY]: indicates that
 *       currently data is ONLY being received from the remote party in the media stream;
 *   - [com.xibasdev.sipcaller.sip.calling.streams.StreamDirection.ENABLED_SEND_ONLY]: indicates that
 *       currently data is ONLY being sent to the remote party in the corresponding media stream;
 *   - [com.xibasdev.sipcaller.sip.calling.streams.StreamDirection.ENABLED_SEND_RECEIVE]: indicates that
 ...
 *   Otherwise, when it is [com.xibasdev.sipcaller.sip.calling.CallDirection.INCOMING], then the local party
 ...
 *
 *   - [com.xibasdev.sipcaller.sip.registering.account.isCaller]
 *   - [com.xibasdev.sipcaller.sip.registering.account.isCallee]
 *
